...
.text
main: 
    la a0, A        # loading the starting addres of array A to a0
...
loop1:
    ...
    add t2, t1, a0  # add i*4 to the base address of A and put it to t2
    lw t3, 0(t2)    # t3 has the value A[i]
    ...
    # restoring a and a1 to their origin value
    lw a0, 0(sp)
    lw a1, 4(sp)
    addi sp, sp, 8
